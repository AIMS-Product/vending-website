...
- 2026-06-10 (N9 execution): migration 20260610100000 applied to PROD via supabase db push (only pending migration, verified no strays). database.ts updated by HAND-ADDING only the new RPC Functions entry — a full `supabase gen types` regen with the local CLI v2.75 drifts unrelated column nullability and breaks ~5 files (committed types were generated by a different CLI version). A clean full regen needs a CLI bump first (v2.105 available) — deferred as out of scope. Orchestrator ran n5's scoped teardown SQL via Supabase Management API for the one stuck probe row (page + 1 publish revision, exact UUID only); verified 0 rows left and no-delete trigger re-enabled ("O").
