...
// Three connectors, each a few requests and a few thousand upserted rows. A
...
export async function GET(request: Request) {
  if (!config.CRON_SECRET) {
    return NextResponse.json(
      { ok: false, message: "Channel sync runner is not configured." },
      { status: 503 },
    );
  }
  ...
  try {
    ...
    console.error("channel sync runner: invalid options", {
  ...
  try {
    const result = await syncChannelDaily(options);
    ...
    return NextResponse.json(
      { ok: false, message: "Channel sync runner failed." },
      { status: 500 },
    );
  }
}
