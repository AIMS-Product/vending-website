...
import { bulkArchiveCaseStudiesFromList } from "@/app/admin/case-studies/list-actions";

// Mirrors NewsBulkArchiveControls. Checkboxes (name="ids") live in the
// server-rendered table rows and associate to this form via the shared
// `form="case-study-bulk-archive-form"` attribute. This client island just
// reflects the live selection count and confirms before submitting — no
// bulk framework.
const FORM_ID = "case-study-bulk-archive-form";

export function CaseStudyBulkArchiveControls({
  returnTo,
}: {
  returnTo: string;
}) {
  ...
  // associate via form="case-study-bulk-archive-form") resolve — we just
  // collapse it to nothing visible until at least one row is selected.
  return (
    <form
      id={FORM_ID}
      action={bulkArchiveCaseStudiesFromList}
        ...
        if (
          !window.confirm(
            `Archive ${selectedCount} selected ${
              selectedCount === 1 ? "case study" : "case studies"
            }? They will be removed from the active list. You can restore them later.`,
          )
        ) {
