...
describe("createQualificationIntakeSession", () => {
  ...
  it("stores a local lead, creates a tokenized qualification session, and queues Close sync", async () => {
    ...
    const result = await createQualificationIntakeSession(
      {
        ...
        firstLandingUrl:
          "https://www.vendingpreneurs.com/resources/start-vending?campaign_id=camp-123",
        firstLandingPath: "/resources/start-vending",
        firstReferrer: "https://www.google.com/",
        firstTouchAt: "2026-06-17T08:55:00.000Z",
        latestLandingUrl: "https://www.vendingpreneurs.com/start-vending",
        latestLandingPath: "/start-vending",
        latestReferrer:
          "https://www.vendingpreneurs.com/resources/start-vending",
    ...
    expect(fake.state.leads[1]).toMatchObject({
      ...
      metadata: expect.objectContaining({
        attribution_session: expect.objectContaining({
          ...
          latest_landing_path: "/start-vending",
          clicked_href: "/start-vending",
        }),
    ...
    expect(fake.state.sessions[0]).toMatchObject({
      ...
      utm_source: "google",
      utm_medium: "cpc",
      consent_source_attribution: expect.objectContaining({
        vp_session_id: "vp-session-1",
        first_landing_path: "/resources/start-vending",
        latest_landing_path: "/start-vending",
        clicked_href: "/start-vending",
