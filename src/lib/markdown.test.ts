...
describe("renderMarkdown GitHub-flavoured option", () => {
  const TABLE = "| a | b |\n| --- | --- |\n| 1 | 2 |\n";

  it("leaves stored body content on the plain pipeline by default", async () => {
    // Every news post and SEO page body renders through this call. Turning
    // GFM on for all of them would silently change published content, so the
    // default must stay non-GFM.
    expect(await renderMarkdown(TABLE)).not.toContain("<table>");
  });

  it("renders tables when a caller opts in", async () => {
    // /admin/data renders REPORTING.md, which is mostly tables: without this
    // the glossary is 265 stray pipe characters.
