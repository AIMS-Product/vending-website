...
- Every analytics tab and `/admin/data` opens with a trust bar (`DataTrustBar`). Which feeds a tab reads is `TAB_FEEDS` and which number an audit check verifies is `CHECK_COVERS`, both in `src/lib/analytics/data-trust-bar.ts`; change them in the same commit as a loader that starts reading a new table or a new audit check. A check that fails, errors or is skipped marks its number "Unverified"; no stored audit run in 26h reads red.
