...
- The chatbot dashboard counts **sales chats only**: `triageConversation` (`src/lib/chatbot/triage.ts`) drops member-support chats, and a chat whose lead's `call_booked_at` is an earlier day than the chat (Pacific) is left out as booked-before-chat. `lead_submissions.call_booked_at` is Close's "First Sales Call Booked Date", a DATE, so a same-day pre-chat booking still counts. Hand-offs live in `chatbot_follow_up_tasks` too and are told apart by the `flag_for_team:` dedupe prefix, never by `task_type`. On 2026-09-23 the 30-day view went 190 chats / 45 booked to 180 / 42 (9 support, 1 booked before).
